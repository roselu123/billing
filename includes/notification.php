...
if(isset($_GET['notification_clicked'])) {
    ...
    $update_query_billing = "UPDATE billing SET notification_read = 1 WHERE Due_Date = '{$currentDate}'";
...
$query_billing = "SELECT id, Patient_Name, Amount, Due_Date
                FROM billing 
                WHERE Due_Date = '{$currentDate}' AND notification_read = 0";

// Execute query for notifications from billing table
$result_billing = mysqli_query($conn, $query_billing);
$notifications = array();
while ($row = mysqli_fetch_assoc($result_billing)) {
    // Insert retrieved data into the notif table
    $insert_query_notif = "INSERT INTO notif (Patient_Name, Amount, Due_Date, no_of_notifications, notification_read) 
                           VALUES ('{$row['Patient_Name']}', '{$row['Amount']}', '{$row['Due_Date']}', 0, 0)";
    mysqli_query($conn, $insert_query_notif);
                    ...
                    <div class="notification-ui_dd-content">
                        ...
                        if ($result_notif && mysqli_num_rows($result_notif) > 0) {
                            while ($row = mysqli_fetch_assoc($result_notif)) {
                                ...
                                if ($row['paid'] == 0) {
                                    echo "<button type='button' class='btn btn-success' data-id='{$id}' onclick='markAsPaid($id)'>Paid</button>";
                                } else {
